...
$TITLE Two-Stage Stochastic Generation Expansion Planning - Using Benders (Multicuts)
...
PARAMETERS
   ...
   pDelta          (l,    sc) "cut type (feasibility 0 optimality 1)      in iteration l"
   pZ2_L           (l,    sc) "subproblem objective function value        in iteration l"
;
* ========================================================================
* VARIABLES
* ========================================================================
...
* Complete problem equations 
   ...
   eOperaCost(    sc) "Total operating  Cost               [kEUR]"   
...
* equations for Benders decomposition
   ...
   eBdCuts   (l,  sc) "Benders cuts at iteration l and scenario  "
;
eZ1..        vZ1        =E= vInvesCost + SUM[sca(sc), pScProb(sc) * vTheta    (sc)]
;                       
eZ2..        vZ2        =E=            + SUM[sca(sc),               vOperaCost(sc)]
;
eTotalCost.. vTotalCost =E= vInvesCost + SUM[sca(sc), pScProb(sc) * vOperaCost(sc)]
;
eInvesCost.. vInvesCost =E= SUM[g, pGenInfo(g,'InvCost')*pGenInfo(g,'UnitCap')*vInstalUnits(g)]
;
eOperaCost(sca(sc))..
    vOperaCost(sc) =E= pWeight * SUM[(p,g),
...
eBdCuts(ll,sca(sc))..
    pDelta(ll ,sc)* vTheta(sc)
        =G= + pZ2_L(ll,sc) 
            - SUM[(p,r)           , pDualRenProd_L(ll,p,r,sc) * pRenProf(p,r,sc) * pGenInfo(r,'UnitCap') * [pInstalUnits_L(ll,r) - vInstalUnits(r)]]
            - SUM[(p,g)$[NOT r(g)], pDualMaxProd_L(ll,p,g,sc)                    * pGenInfo(g,'UnitCap') * [pInstalUnits_L(ll,g) - vInstalUnits(g)]]
;
...
*  storing parameters to build a new Benders' cut
   if (Subproblem_Bd.ModelStat = 4,
      pDelta(l,sc) = 0 ;
      pZ2_L (l,sc) = Subproblem_Bd.SumInfes ;
   else
*     updating lower and upper bound
      pZ_Lower =              vZ1.L ;
      pZ_Upper = MIN(pZ_Upper,vZ1.L - SUM[sc, pScProb(sc) * vTheta.L(sc)] + SUM[sc, pScProb(sc) * vOperaCost.L(sc)]) ;

      pDelta(l,sc) =     1 ;
      pZ2_L (l,sc) = vOperaCost.L(sc) ;
   ) ;
...
;
pScPrices (p,sca(sc)) =                       eBalance.M(p,sc)  *1e3 / [pWeight];
pEVPrices (p        ) = SUM[sc, pScProb(sc) * pScPrices (p,sc)]                 ;

* gdx with all results
execute_unload 'TwoStageStochGEP-Benders-Multicut.gdx'
